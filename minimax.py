...
def evaluate_heuristic(board):
    ...
    for line in lines:
        if line.count('X') == 2 and line.count('') == 1:
            score += 3  # Opportunity to win
        if line.count('O') == 2 and line.count('') == 1:
            score -= 4  # Need to block opponent
        if line.count('X') == 1 and line.count('') == 2:
            score += 1  # Good early formation
        if line.count('O') == 1 and line.count('') == 2:
            score -= 1  # Opponent early formation
    return score
    ...
    for i, row in enumerate(board):
        for j, box in enumerate(row):
            if box == '':
                ...
                # Heuristic bonus
                if depth == 0:
                    score += evaluate_heuristic(board_copy)
