"""
Heuristic for providing bonus scores for "two-in-a-row" opportunity and center bonus
"""
import sys
import copy
import random
...
def count_twos_and_center(player, board):
    twos = 0
    center_bonus = 1 if board[1][1] == player else 0
    lines = []
...
def evaluate(board):
    if check_tris('X', board):
        ...
        score = count_twos_and_center('X', board) - count_twos_and_center('O', board)
        return score

def minimax(max_turn, board, depth=0, alpha=float('-inf'), beta=float('inf')):
    if check_tris('X', board) or check_tris('O', board) or game_tie(board):
